...
var requestToApi = function requestToApi(args) {
  ...
  var returnData = function () {
    var _ref2 = asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee2(request, resolve, reject, isCancel) {
...
var Fetch = function (_Component) {
  ...
  function Fetch() {
    ...
    return _ret = (_temp = (_this = possibleConstructorReturn(this, _Component.call.apply(_Component, [this].concat(args))), _this), _this._data = undefined, _this._didCallOnLoad = false, _this._isLoaded = false, _this._isUnmounted = false, _this._fetchData = function () {
      var _ref = asyncToGenerator( /*#__PURE__*/regenerator.mark(function _callee(props) {
        var body, cancel, context, headers, method, onIntercept, onProgress, onTimeout, params, path, url, timeout, route, timeoutValue, apiResponse;
        return regenerator.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                body = props.body, cancel = props.cancel, context = props.context, headers = props.headers, method = props.method, onIntercept = props.onIntercept, onProgress = props.onProgress, onTimeout = props.onTimeout, params = props.params, path = props.path, url = props.url, timeout = props.timeout;
                route = void 0;
                timeoutValue = 0;


                if (path) route = '' + (context.api || '') + path;else route = url;

                if (context.timeout && timeout === -1) timeoutValue = context.timeout;else if (!context.timeout && timeout) timeoutValue = Math.max(0, timeout);else if (context.timeout && timeout) timeoutValue = timeout === -1 ? context.timeout : timeout;
      ...
      return error.response && JSON.stringify(error.response).length ? typeof error.response === 'string' ? error.response : _typeof(error.response) === 'object' ? JSON.stringify(error.response, null, 2) : error.response + '. Sorry <Fetch /> couldn\'t turned this into a readable string. ' + 'Check error.content.request to see what happened.' : // $FlowFixMe
      error.request._response ? typeof error.request._response === 'string' ? error.request._response : _typeof(error.request._response) === 'object' ? JSON.stringify(error.request._response, null, 2) : String(error.request._response) + '. Sorry <Fetch /> couldn\'t turned this into a readable string. ' + 'Check error.content.request to see what happened.' : " .Sorry <Fetch /> couldn't turned this into a readable string. " + 'Check error.content.request to see what happened.';
      ...
      if (context.loader && !loader) {
        return typeof context.loader === 'function' ? context.loader() : context.loader;
      }
      if (!context.loader && loader) return typeof loader === 'function' ? loader() : loader;
      if (context.loader && loader) return typeof loader === 'function' ? loader() : loader;
    ...
    }, _this._validateProps = function (props) {
      ...
      if (path) {
        !(path && context.api) ? process.env.NODE_ENV !== 'production' ? invariant_1(false, 'You must implement <FetchProvider> at the root of your ' + 'app and provide a URL to `value.api` prop in order to use `path`') : invariant_1(false) : void 0;
      }

      if (path === 'store') {
        !(path && context.store) ? process.env.NODE_ENV !== 'production' ? invariant_1(false, 'You must implement <FetchProvider> at the root of your ' + 'app and provide an object to `value.store` prop ' + 'in order to use `store`') : invariant_1(false) : void 0;
      }

      if (onTimeout) {
        !(typeof timeout === 'number' && timeout >= 0 || typeof context.timeout === 'number' && context.timeout >= 0) ? process.env.NODE_ENV !== 'production' ? invariant_1(false, 'You must provide a `timeout` number in ms to <Fetch /> or ' + '<FetchProvider> in order to use `onTimeout`') : invariant_1(false) : void 0;
      }
  ...
  Fetch.prototype.UNSAFE_componentWillMount = function UNSAFE_componentWillMount() {
  ...
  Fetch.prototype.render = function render() {
    ...
    if (this._isLoaded && !this._isUnmounted) {
      if (component) return _react.createElement(component, this._data);

      if (typeof render === 'function') return render(this._data);

      if (typeof children === 'function') return children(this._data);

      if (children && !isEmptyChildren(children)) return _react.Children.only(children);
    }
...
var withContext = function withContext(FetchComponent) {
  return function (props) {
    return _react__default.createElement(
      ...
      null,
      function (data) {
        return _react__default.createElement(FetchComponent, _extends({}, props, { context: data }));
      }
    );
  };
};
...
exports.FetchProvider = Provider;
